...
const MyProfile = () => {
  ...
  const updateUserPrpfileData = async () => {
    try {
  ...
  return userData && (
    <div className="max-w-lg flex flex-col gap-2 text-sm">
      ...
      {isEdit ? (
        <input
        className="bg-gray-200 text-3xl font-medium max-w-64 mt-4"
          type="text"
          value={userData.name}
          onChange={(e) =>
      ...
      <div>
        ...
        <div className="grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700">
          ...
          {isEdit ? (
            <select className="max-w-20 bg-gray-200 "
              onChange={(e) =>
          ...
          {
            isEdit ? 
            <input className="max-w-28 bg-gray-200" type="date" onChange={(e) =>
      ...
      <div className="mt-10">
        {
          isEdit ? 
          <button className="border border-[#5f6FFF] px-8 py-2 rounded-full hover:bg-[#5f6FFF] hover:text-white transition-all " onClick={updateUserPrpfileData}>Save information</button>
